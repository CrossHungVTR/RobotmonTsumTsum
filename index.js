...
var Page = {
  ...
  GamePlaying2: {
    name: 'GamePlaying',
    colors: [
      {x: 980, y:  186 + adjY, r: 244, g: 197, b: 5, match: true, threshold: 80},
      {x: 916, y: 1616 + adjY, r: 230, g: 150, b: 6, match: true, threshold: 80},
    ],
    back: {x: 986, y: 201 + adjY},
    next: {x: 986, y: 201 + adjY},
...
function recognizeGameTsums(boardImg, allTsumImages, myTsum, isJP, debug, logs) {
  ...
  // Remove same Tsums
  removeSameTsumImages(gameTsums, 0.925);
...
//    if(autoPlay){gTaskController.newTask('taskPlayGameQuick', ts.taskPlayGameQuick.bind(ts), 3 * 60 * 1000, 0);}
  } else {
    if (!isPause) {
      log(ts.logs.updateApp);
      sleep(1000);
    }
    if(autoPlay){gTaskController.newTask('taskPlayGame', ts.taskPlayGame.bind(ts), 3 * 60 * 1000, 0);}
  }
  sleep(500);
